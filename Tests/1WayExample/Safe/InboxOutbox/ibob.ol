...
from ..test.testTypes import ProducerTestInterface, ConsumerTestInterface, TestExceptionType
...
service IBOB(p: IBOBConfig){
    ...
    outputPort InboxWriter {
        Location: "local"  // overwritten in init
        Interfaces: InboxWriterExternalInterface, ConsumerTestInterface
    }

    outputPort InboxReader {
        Location: "local"
    ...
    inputPort IBOB {
        ...
        Aggregates: Outbox, InboxWriter, InboxReader
    }
    ...
    define _setupInboxDefaultValues{
        if ((!is_defined(p.inboxConfig.pollTimer)) || p.inboxConfig.pollTimer < 0){
    ...
    define _setupOutboxDefaultValues{
        if ((!is_defined(p.outboxConfig.pollTimer)) || p.outboxConfig.pollTimer < 1){
